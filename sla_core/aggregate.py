...
def kpi(df: pd.DataFrame, company: str | None = None,
    ...
    """Core KPI output for the selected filters."""
    sub = filter_frame(df, company, month)
    total = len(sub)
    within = int((sub["sla_within_flag"] == "Y").sum())
...
def by_company(df: pd.DataFrame, month: str | None = None) -> pd.DataFrame:
    ...
    g = sub.assign(_within=(sub["sla_within_flag"] == "Y").astype(int))
...
def by_month(df: pd.DataFrame, company: str | None = None) -> pd.DataFrame:
